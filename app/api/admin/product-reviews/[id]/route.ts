...
// DELETE - Delete a product review and recalculate product ratings
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Get the review to find the productId
    ...
    // Delete review and recalculate product stats in a transaction
    await prisma.$transaction(async (tx) => {
      ...
      const remainingReviews = await tx.productReview.findMany({
        where: {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Update review and recalculate product stats if approval/visibility changed
    await prisma.$transaction(async (tx) => {
      // Update the review
      ...
      if (typeof isHidden === "boolean" || typeof isApproved === "boolean") {
        const activeReviews = await tx.productReview.findMany({
          where: {
            productId: review.productId,
            isApproved: true,
            isHidden: false,
